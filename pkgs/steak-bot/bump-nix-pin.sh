...
Update the local steak-bot package.nix next to this script to the current
discord-relay HEAD, commit the change in the nix-config repo, and optionally
push it.
...
git -C "$NIX_REPO" commit -m "steak-bot: bump to ${new_rev:0:12}"
