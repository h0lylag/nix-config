...
let
  ...
  instanceConfig = types.submodule {
    options = {
      ...
      user = mkOption {
        ...
        default = null;
        description = "User to run qBittorrent-nox as (defaults to services.qbittorrent-nox.global.user)";
      };

      group = mkOption {
        type = types.nullOr types.str;
        default = null;
        description = "Group to run qBittorrent-nox as (defaults to services.qbittorrent-nox.global.group)";
      };
      ...
      dataDir = mkOption {
        type = types.nullOr types.str;
        default = null;
        description = "Base directory for qBittorrent data (defaults to services.qbittorrent-nox.global.dataDir)";
      };
  ...
  mkService =
    name: instanceCfg:
    let
      # Apply global fallbacks for common fields
      user = if instanceCfg.user != null then instanceCfg.user else cfg.global.user;
      group = if instanceCfg.group != null then instanceCfg.group else cfg.global.group;
      dataDir = if instanceCfg.dataDir != null then instanceCfg.dataDir else cfg.global.dataDir;

      profileDir = "${dataDir}/${instanceCfg.profile}";
    ...
    in
    {
      ...
      serviceConfig = {
        ...
        User = user;
        Group = group;
        ExecStart = "${cfg.package}/bin/qbittorrent-nox ${args}";
        Restart = "always";
  ...
  mkTmpfiles =
    ...
    in
    [
      "d ${profileDir} 0750 ${user} ${group} -"
      "d ${savePath} 0770 ${user} ${group} -"
      "d ${incompletePath} 0770 ${user} ${group} -"
    ]
    ++ (optional (watchPath != null) "d ${watchPath} 0770 ${user} ${group} -")
    ++ (optional (
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    # Open firewall ports for all enabled instances (configurable)
